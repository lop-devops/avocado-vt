...
class LibvirtPolkitConfig(object):
    ...
    def __init__(self, params):
        ...
        if not self.test_type:
            ...
            if "negative_test" in test_name:
                ...
                # Check if any ACL-specific error flags are set
                # (e.g., destroy_error, define_error, undefine_error)
                acl_error_flags = [
                    "destroy_error",
                    "define_error",
                    "undefine_error",
        ...
        if libvirt_version.version_compare(5, 6, 0) and self.params.get(
            ...
            # e.g. "connect_driver:QEMU|storage" means allow QEMU and storage.
            # Expand into separate attrs so the polkit rule allows any with OR logic.
            expanded_attrs = []
            for attr in self.attr:
                conn_driver = re.findall(r"connect_driver:(.+)\|(.+)", attr)
                if conn_driver:
                    # Regex captures two groups: before | and after |.
                    # e.g. "connect_driver:QEMU|storage" -> ('QEMU', 'storage')
                    ...
                    LOG.debug(
                        "Expanded connect_driver '%s' to: ['%s', '%s']",
                        attr,
                        driver1,
                        driver2,
                    )
    ...
    def _set_polkit_conf(self):
        ...
        LOG.info(
            "Positive ACL test detected (test_type='%s'): Creating polkit rule",
            self.test_type,
        )
        ...
        try:
            ...
            if libvirt_version.version_compare(5, 6, 0) and self.action_list:
                ...
                if self.attr:
                    # Group attributes by key for mixed OR/AND logic.
                    # e.g. "connect_driver:QEMU connect_driver:network network_name:x"
                    # becomes: (connect_driver==QEMU || connect_driver==network)
                    ...
                    for key, values in attr_groups.items():
                        if len(values) == 1:
                            ...
                            conditions.append(
                                f"action.lookup('{key}') == '{values[0]}'"
                            )
                        else:
                            # Multiple values for same key, use OR
                            or_conditions = [
                                f"action.lookup('{key}') == '{v}'" for v in values
                            ]
                            conditions.append("(" + " || ".join(or_conditions) + ")")
                            LOG.debug(
                                "Using OR logic for multiple '%s' values: %s",
                                key,
                                values,
                            )
                ...
                if not self.action_id:
                    ...
                    # Covers all org.libvirt actions: unix.manage, unix.monitor, API.
                    rules = "    if (action.id.indexOf('org.libvirt') == 0 &&\n"
                    rules += f'        subject.user == "{self.user}") {{\n'
                    rules += "        return polkit.Result.YES;\n"
                    rules += "    }\n"
                    ...
                    for i in range(len(self.action_id)):
                        ...
                        # For connect actions, use indexOf to match all connect.*
                        if action_id.startswith("org.libvirt.api.connect."):
                            action_match = (
                                "action.id.indexOf" "('org.libvirt.api.connect') == 0"
                            )
                            LOG.debug(
                                "Using indexOf match for connect action: %s",
                                action_id,
                            )
                        else:
